...
#'
mod_download_plot_ui <- function(id, label = "Download Plot") {
  ns <- NS(id)
  div(
...
#'
mod_download_plot_server <- function(id, filename, figure, width = 8, height = 6) {
  moduleServer(id, function(input, output, session) {
    ...
    figure_width <- reactive({
      if (is.numeric(input$width) && !is.na(input$width)) {
        # cutoff the input value to a range as shiny does not enforce
        return(max(min_size, min(max_size, input$width)))
      } else {
        # use default value when entered text or NA
        return(width)
      }
    })
    ...
    figure_height <- reactive({
      if (is.numeric(input$height) && !is.na(input$height)) {
        # cutoff the input value to a range as shiny does not enforce
        return(max(min_size, min(max_size, input$height)))
      } else {
        # use default value when entered text or NA
        return(height)
      }
